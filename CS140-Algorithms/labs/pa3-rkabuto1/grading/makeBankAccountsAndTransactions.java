class bailey_makeBankAccountsAndTransactions
{
		...
		try
		{
			...
			for( int i = 0; i < inputLines.size(); )
			{
				...
				if( inn.startsWith("new BankAccount") )
				{
					...
					bailey_BankAccount ba = new bailey_BankAccount(lastName, firstName, checkingInitialValue, savingsInitialValue);
				...
				if( inn.startsWith("new EnhancedBankAccount") )
				{
					...
					double checkingInitialValue = Double.parseDouble(st.nextToken());
					double savingsInitialValue = Double.parseDouble(st.nextToken());
					bailey_EnhancedBankAccount eba = new bailey_EnhancedBankAccount(lastName, firstName, checkingInitialValue, savingsInitialValue);
