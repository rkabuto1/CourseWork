...
class BinarySearchTreeTest 
{
	...
	{
		// delete the nodes with keys 45, 95, 60, and 95 from the 31 key treeƒ
	...
	{
		int[] keys = new int[] {20, 10, 30,35, 5, 35, 15, 35, 35, 35};
        for( int i = 0; i < keys.length; i++ )
        {
            Node n = new Node(keys[i]);
            tree.insertNode(n);
        }
        assertEquals(4 ,t.getNode(t.getRoot(),35).getCount());
	}
	...
	void testCountForDuplicateNodeWithDelete() 
	{
        ...
        assertEquals(2 ,t.getNode(t.getRoot(),35).getCount());
	}	
}