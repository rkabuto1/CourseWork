...
int policy = 0; // 0: default, 1: stride
...
int fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p->state == RUNNABLE || p->state == RUNNING)
    {
      p->tickets = STRIDE_TOTAL_TICKETS / (num_active_processes);
...
void exit(void)
{
  ...
  {
    if (x->state == RUNNABLE || x->state == RUNNING)
    {
      num_active_processes++;
    }
  }
  ...
  for (x = ptable.proc; x < &ptable.proc[NPROC]; x++)
  {
    if (x->state == RUNNABLE || x->state == RUNNING)
    {
      x->tickets = STRIDE_TOTAL_TICKETS / (num_active_processes);
  ...
  for (;;)
  {
    ...
    else // stride scheduling
    {
      ...
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        ...
        if (!min_proc || p->pass < min_proc->pass)
        {
          min_proc = p;
        }
      }
      if (min_proc)
      {
        min_proc->pass += min_proc->strides;
        ran = 1;
        c->proc = min_proc;
        switchuvm(min_proc);
