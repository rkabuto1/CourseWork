...
double bruteForce2D(vector<Point2D>& points, int& i_min, int& j_min) {
    double minimum = DBL_MAX;
    for (size_t i = 0; i < points.size(); ++i) {
        for (size_t j = i + 1; j < points.size(); ++j) {
            double distNew = d2(points[i], points[j]);
            if (distNew < minimum) {
                minimum = distNew;
                i_min = i;
                j_min = j;
...
double bruteForce3D(vector<Point3D>& points, int& i_min, int& j_min) {
    double minimum = DBL_MAX;
    for (size_t i = 0; i < points.size(); ++i) {
        for (size_t j = i + 1; j < points.size(); ++j) {
            double distNew = d3(points[i], points[j]);
            if (distNew < minimum) {
                minimum = distNew;
                i_min = i;
                j_min = j;
...
double stripMin2D(vector<Point2D>& vec, double dist, int& i_min, int& j_min) {
    double minD = dist;
    sort(vec.begin(), vec.end(), [](const Point2D& a, const Point2D& b) { return a.y < b.y; });
...
double util2D(vector<Point2D>& pts, vector<Point2D>& tmp, int l, int r, int& i_min, int& j_min) {
    if (r - l > 1) {
        ...
        for (int i = l; i < r; ++i) {
            if (fabs(pts[i].x - midPt.x) < minimum) {
                stripPts.push_back(pts[i]);
            }
        }
        double stripMinDist = stripMin2D(stripPts, minimum, i_min, j_min);
        return minVal(minimum, stripMinDist);
    }
    return DBL_MAX;
}
...
double util3D(vector<Point3D>& pts, vector<Point3D>& tmp, int l, int r, int& i_min, int& j_min) {
    if (r - l > 1) {
        ...
        double stripMinDist = stripMin3D(stripPts, minimum, i_min, j_min);
        return minVal(minimum, stripMinDist);
    }
    return DBL_MAX;
}
